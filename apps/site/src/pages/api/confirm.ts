...
export async function POST({ request }: APIContext) {
  ...
  const apiToken = env.LIST_API_TOKEN;
  ...
  if (!isConfirmationToken(body.token) || !turnstileToken) {
    return json({ error: "Invalid confirmation link." }, 400);
  }

  const requestUrl = new URL(request.url);
  const browserOrigin = request.headers.get("origin");
  const originHostname = browserOrigin
    ? new URL(browserOrigin).hostname
    : undefined;
  const hostHostname = request.headers.get("host")?.split(":")[0];
  const browserHostname = [originHostname, hostHostname, requestUrl.hostname].find(
    (hostname) => hostname && isLocalHostname(hostname),
  ) ?? originHostname ?? hostHostname ?? requestUrl.hostname;
  const remoteIp = request.headers.get("cf-connecting-ip") ?? undefined;
  const verification = await verifyTurnstile({
    secret: turnstileSecret,
    response: turnstileToken,
    expectedAction: TURNSTILE_ACTION,
    expectedHostname: browserHostname,
    allowTestKey: env.TURNSTILE_TEST_MODE === "true",
    ...(remoteIp ? { remoteIp } : {}),
  });
    ...
    const upstream = await fetch(
      ...
      ),
      {
        ...
        body: JSON.stringify({
          ...
          sourceUrl: `${browserOrigin ?? requestUrl.origin}/confirm`,
        }),
      },
    );
